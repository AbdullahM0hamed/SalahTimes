...
public class QiblahFragment extends Fragment implements SensorEventListener {
  ...
  void setSunImage(MainActivity activity) {
    Date today = new Date();
    PrayerTimes times =
        CommonCode.getPrayerTimes(activity, today, activity.latitude, activity.longitude);
    int solarImage =
        (today.getTime() > times.dhuhr.getTime())
            ? R.drawable.ic_nesw_sun_west
            : R.drawable.ic_nesw_sun_east;
  ...
  float getTrueNorthAngle(MainActivity activity) {
    ...
    GeomagneticField geoField =
        new GeomagneticField(
            (float) activity.latitude, (float) activity.longitude, 0, (new Date()).getTime());
